...
export const SET_USER_FETCH_PARAMS = "SET_USER_FETCH_PARAMS";
...
const innerFetchUsers = _.debounce((dispatch, getState) => {
  ...
  Axios.get(fetchDataUrl, { params: getState().userDataTable.params }).then(
    result => {
      dispatch(
        fetchUsersSuccess({
...
const setFetchParamsInternal = params => {
  return {
    type: SET_USER_FETCH_PARAMS,
    payload: params
  };
};

export const setFetchParams = params => (dispatch, getState) => {
  const newParams = { ...getState().userDataTable.params, ...params };
